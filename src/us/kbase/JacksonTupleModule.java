...
public class JacksonTupleModule extends SimpleModule {
	public JacksonTupleModule() {
		super(JacksonTupleModule.class.getSimpleName(), new Version(1, 0, 0,
				null));
		...
		setDeserializers(new SimpleDeserializers() {
			...
			public JsonDeserializer<?> findBeanDeserializer(JavaType type,
				...
				return super.findBeanDeserializer(type, config, provider,
						beanDesc, property);
			}
		});
	}
		...
		public Object deserialize(JsonParser p, DeserializationContext ctx)
			...
			try {
				...
				if (!p.isExpectedStartArrayToken()) {
					System.out.println("Bad parse in TupleDeserializer: "
							+ p.getCurrentToken());
					return null;
				}
				p.nextToken();
				for (int i = 0; i < types.size(); i++) {
					Method m = res.getClass().getMethod("setE" + (i + 1),
							Object.class);
