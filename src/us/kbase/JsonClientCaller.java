...
public class JsonClientCaller {
	...
	private static Map<String, AuthToken> user2token = Collections
			.synchronizedMap(new HashMap<String, AuthToken>());
	...
	public JsonClientCaller(String url, String accessToken)
			throws MalformedURLException, IOException {
		this(url);
		this.accessToken = new AuthToken(accessToken);
	}

	public JsonClientCaller(String url, String user, String password)
		...
		if (authRequired || user != null || accessToken != null) {
			if (!(conn instanceof HttpsURLConnection || isAuthAllowedForHttp)) {
				throw new IllegalStateException(
						"RPC method required authentication shouldn't be called through unsecured http, "
			...
			if (accessToken == null) {
				if (user == null) {
					if (authRequired)
						throw new IllegalStateException(
								"RPC method requires authentication but neither user nor token was set");
				} else {
					accessToken = user2token.get(user);
					if (accessToken != null
							&& !checkTokenExpirationDate(accessToken)) {
					...
					if (accessToken == null) {
						try {
							...
							try {
								accessToken = requestTokenFromGlobus(user,
										password);
							} catch (Exception e2) {
								if (authRequired)
									throw e2;
	...
	private static AuthToken requestTokenFromGlobus(String user, char[] password)
			throws Exception, MalformedURLException, ProtocolException,
		...
		HttpURLConnection authConn = (HttpURLConnection) new URL(authUrl)
				.openConnection();
		String credential = DatatypeConverter
				.printBase64Binary((user + ":" + new String(password))
						.getBytes());
		...
		LinkedHashMap<String, Object> respMap = parser
				.readValueAs(new TypeReference<LinkedHashMap<String, Object>>() {
				});
		return new AuthToken((String) respMap.get("access_token"));
	}

	private static void checkReturnCode(HttpURLConnection conn)
			throws Exception {
		...
		if (responseCode >= 300) {
			...
			br.close();
			throw new IllegalStateException("Wrong server response: code="
					+ responseCode + ", error_message=" + sb);
		}
	}
	...
	public <ARG, RET> RET jsonrpcCall(String method, ARG arg,
		...
		JsonGenerator g = mapper.getJsonFactory().createJsonGenerator(os,
				JsonEncoding.UTF8);
		...
		if (node.has("error")) {
			Map<String, String> ret_error = mapper.readValue(node.get("error"),
