...
find_sites_by_motif_collection_with_mast - search a sequence database for occurences of known motifs by MAST

=head1 SYNOPSIS

#TODO find_sites_by_motif_collection_ws_with_mast [--url=http://kbase.us/services/meme --sequence_set=<sequence set ID> --mt=<mt> ] < motif_collection_meme_object
...
#TODO the following code has not been changed yet
...
my $results = $oc->findSitesByMotifCollectionWithMast($motif_collection, $sequence_set, $mt);
